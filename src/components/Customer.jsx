...
const Customer = ({onViewDetail}) => {
    ...
    const fetchContacts = useCallback(async () => {
        try {
            const response = await apiRequest(
                'get',
                `/api/contact?page=${currentPage}&page_size=${pageSize}&search=${keyword}`,
                null,
                {
            ...
            console.error('Error during fetch contact:', error);
            Swal.fire({
                icon: 'error',
                title: 'Fetch contact error',
    ...
    return (
        <div>
            ...
            <div className="absolute left-[80px] top-[10px] w-[664px] h-[52px] font-poppins font-bold text-[18px] leading-[36px] flex items-center text-white"><pre>{'Dashboard  >  Customer'}</pre></div>
                            ...
                            {contacts.length === 0 ? (
                                ...
                                contacts.map((contact, index) => (
                                    <tr key={contact.id}>
                                        ...
                                        <td className="py-2 px-4 border-b border-blue-200">{contact.name}</td>
                                        <td className="py-2 px-4 border-b border-blue-200">{contact.phone}</td>
                                        <td className="py-2 px-4 border-b border-blue-200">{contact.email}</td>
                                        <td className="py-2 px-4 border-b border-blue-200">{contact.created_at.replace(/[TZ]/g, ' ')}</td>
                                        <td className="py-2 px-4 border-b border-blue-200"><FontAwesomeIcon icon={faEye} className='hover:cursor-pointer h-[20px]' onClick={() => onViewDetail(contact.id)} /></td>
                                    </tr>
                                ))
                            )}
                        </tbody>
